...
$(window).on('load', function () {
	...
	(function () {
		let moreText = document.querySelector('#bx_719294866_27795 .container');
		if (moreText) {
			let button = document.createElement('div');
			let openText = 'Читать подробнее',
				closeText = 'Свернуть';
		...
		let moreText2 = document.querySelector('#bx_719294866_27797 .container');
		if (moreText2) {
			let button = document.createElement('div');
			let openText = 'Читать подробнее',
				closeText = 'Свернуть';
			moreText2.append(button);
			button.classList.add('js-switch');
			button.innerHTML = openText
			button.onclick = function () {
				this.parentElement.classList.toggle('active');
